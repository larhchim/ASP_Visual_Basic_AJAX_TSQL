...
<%
    ...
    If(Ope = "info") Then
        If((Len(Nom) < 2) Or (Len(Prenom) < 2) Or (Len(Email) < 7) Or (Len(Tel) < 10))  Then
            Session("Msg") = "Prière de saisir correctement les informations demandées."
	        Con.Close()
	        Set Con = Nothing
            Response.Redirect "Message.asp"
        End If
        Req = "ps_user_update_infos " & Session("IdUser") & ",'" & Nom & "','" & Prenom & "','" & Email & "','" & Tel & "'"
        Con.Execute Req
        Con.Close()
	    Set Con = Nothing
    ...
    ElseIf(Ope = "pass") Then
        ...
        if(Session("Passwd") = OldPass) Then
            Req = "ps_user_update_pass " & Session("IdUser") &  ", '" & NewPass & "'"
            Con.Execute Req
            Con.Close()
	        Set Con = Nothing
        ...
        Else
            Con.Close()
	        Set Con = Nothing
            Session("Msg") = "Votre ancien mot de passe est erroné."
            Response.Redirect "Message.asp"           
        End If
    Else
