...
<%
	...
	IdCompte = Replace(Request.Form("IdCompte"), "'", "''")
	Nom = FormatName(Replace(Request.Form("Nom"), "'", "''"))
	Prenom = FormatName(Replace(Request.Form("Prenom"), "'", "''"))
	Tel = DelSpace(Replace(Replace(Request.Form("Tel"), "'", "''"), "%2B", "+"))
	Email = DelSpace(Replace(Request.Form("Email"), "'", "''"))
	MP = Replace(Request.Form("MP"), "'", "''")
	Act = LCase(Replace(Request.Form("Act"), "'", "''"))
	...
	ElseIf(Act="true") Then
		Act = 1
	Else
		Response.Write "Valeur activation manquante : " & IdCompte
		Response.end
	End If

	If(IsNumeric(IdCompte)) Then
		Req = "ps_compte_update_infos " & IdCompte & ",'" & Nom & "','" & Prenom & "','" & Tel & "','" & Email & "','" & MP & "'," & Act
	Else
