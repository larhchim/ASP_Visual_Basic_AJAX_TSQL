function AjaxGetListe(fichier) {
	...
	xmlhttp.onreadystatechange = function () {
		if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200)) {
			...
			document.getElementById('LabCpt').innerHTML = 'Montant : (' + T[0] + ')';
...
function Rechercher() {
	...
	fichier = 'GestionDepenseAjaxListe.asp?crt=' + crt + '&val=' + val;
...
function ChargerData(Id) {
	var cible = 'GestionDepenseAjaxGet.asp?IdDepense=' + Id;
	AjaxGetData(cible);
}
